...
import com.denn1996zmq.MyNotepad.ConfirmationDialogFragment.ConfirmationDialogListener;
import com.denn1996zmq.MyNotepad.NotepadApplication;
import com.denn1996zmq.MyNotepad.R;
import com.denn1996zmq.MyNotepad.note_edit.NoteEditActivity;
...
public class NotepadListActivity extends ActionBarActivity implements
    ...
    private void addTile(Note note, ViewGroup parent, LayoutInflater inflater, Animation inAnimation) {
        ...
        //CardView cv = (CardView) child.findViewById(R.id.cardView);
        LinearLayout ll = (LinearLayout) child.findViewById(R.id.tile_clickable);
        TextView tv = (TextView) child.findViewById(R.id.noteTitle);
        tv.setText(note.getText());
        final Note n = note;
        Calendar cal = Calendar.getInstance();
        cal.set(n.mYear, n.mMonth, n.mDay, n.mHour, n.mMinute);
        Calendar today = Calendar.getInstance();
        today.setTime(new Date());
        if (cal.before(today) && n.mYear >= 2000) {
            //cv.setCardBackgroundColor(Color.GRAY)
            ll.setBackgroundColor(Color.rgb(255,100,100));
        }
